...
bool SurfaceParams::CanTexCopy(const SurfaceParams& texcopy_params) const {
    if (pixel_format == PixelFormat::Invalid || addr > texcopy_params.addr || end < texcopy_params.end) {
        return false;
    }
...
void SurfaceParams::UpdateParams() {
    ...
    if (levels != 1) {
        ASSERT(stride == width);
        mipmap_offsets = CalculateMipLevelOffsets(*this);
        size = CalculateSurfaceSize(*this);
    } else {
...
Rect2D SurfaceParams::GetSubRect(const SurfaceParams& sub_surface) const {
    const u32 level = LevelRange(*this, sub_surface.GetInterval()).first;
...
SurfaceParams SurfaceParams::FromInterval(SurfaceInterval interval) const {
    ...
    const u32 level = LevelRange(*this, interval).first;
    params.width >>= level;
    params.stride >>= level;
