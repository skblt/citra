...
void RasterizerCache<T>::CopySurface(SurfaceId src_id, SurfaceId dst_id,
    ...
    ASSERT(dst_params.GetInterval() == copy_interval && src_id != dst_id);
...
auto RasterizerCache<T>::GetFramebufferSurfaces(bool using_color_fb, bool using_depth_fb)
    ...
    const Framebuffer framebuffer{runtime, color, depth_stencil, regs, fb_rect};

    // Mark framebuffer surfaces as dirty
    if (using_color_fb) {
        auto interval = framebuffer.Interval(SurfaceType::Color);
        InvalidateRegion(boost::icl::first(interval), boost::icl::length(interval),
                         color_surface_id);
    }

    if (using_depth_fb) {
        auto interval = framebuffer.Interval(SurfaceType::DepthStencil);
        InvalidateRegion(boost::icl::first(interval), boost::icl::length(interval),
                         depth_surface_id);
    }
...
auto RasterizerCache<T>::GetTexCopySurface(const SurfaceParams& params) -> SurfaceRect_Tuple {
    SurfaceId match_surface_id = FindMatch<MatchFlags::TexCopy>(params, ScaleMatch::Ignore);
...
template <class T>
void RasterizerCache<T>::DuplicateSurface(SurfaceId src_surface_id, SurfaceId dst_surface_id) {
    Surface& src_surface = slot_surfaces[src_surface_id];
    Surface& dst_surface = slot_surfaces[dst_surface_id];
    ...
    for (const auto& pair : RangeFromInterval(dirty_regions, src_surface.GetInterval())) {
        if (pair.second == src_surface_id) {
            regions += pair.first;
        }
    }

    for (const auto& interval : regions) {
        dirty_regions.set({interval, dst_surface_id});
    }
}
...
void RasterizerCache<T>::ValidateSurface(SurfaceId surface_id, PAddr addr, u32 size) {
    ...
    // Fill surfaces will always be valid when used
...
void RasterizerCache<T>::DownloadSurface(Surface& surface, SurfaceInterval interval) {
    ...
    const BufferTextureCopy download = {
        ...
        .texture_level = surface.LevelOf(flush_info.addr),
    };
...
void RasterizerCache<T>::InvalidateRegion(PAddr addr, u32 size, SurfaceId region_owner_id) {
    ...
    for (const SurfaceId remove_surface_id : remove_surfaces) {
        if (remove_surface_id == region_owner_id) {
            ...
            SurfaceId expanded_surface_id =
                FindMatch<MatchFlags::SubRect>(params, ScaleMatch::Ignore);
            ...
            if ((region_owner.invalid_regions - expanded_surface.invalid_regions).empty()) {
                DuplicateSurface(region_owner_id, expanded_surface_id);
            } else {
                continue;
            }
        }
...
template <class T>
auto RasterizerCache<T>::CreateSurface(const SurfaceParams& params) -> SurfaceId {
...
void RasterizerCache<T>::UnregisterAll() {
    FlushAll();
    for (auto& [page, surface_ids] : page_table) {
        while (!surface_ids.empty()) {
            UnregisterSurface(surface_ids.back());
        }
    }
    remove_surfaces.clear();
}
