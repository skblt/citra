...
class RasterizerCache : NonCopyable {
    ...
    using SurfaceRect_Tuple = std::tuple<SurfaceId, Common::Rectangle<u32>>;
...
public:
    ...
    /// Load a texture from 3DS memory and cache it (if not already cached)
    SurfaceId GetSurface(const SurfaceParams& params, ScaleMatch match_res_scale,
                         bool load_if_create);

    /// Attempt to find a subrect (resolution scaled) of a surface, otherwise loads a texture from
    /// 3DS memory and caches it (if not already cached)
...
private:
    ...
    void DuplicateSurface(SurfaceId src_surface_id, SurfaceId dest_surface_id);
