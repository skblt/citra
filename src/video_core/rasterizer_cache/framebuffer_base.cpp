...
FramebufferBase::FramebufferBase(const Pica::Regs& regs, SurfaceBase* const color,
    ...
    const auto Prepare = [&](SurfaceBase* const surface) {
        if (!surface) {
        ...
        intervals[cursor++] = surface->GetSubRectInterval(draw_rect_unscaled);
    };

    // Query surface formats and invalidation intervals
    Prepare(color);
